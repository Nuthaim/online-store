...
router.put('/users/:id/role', superAdminAuth, async (req, res) => {
  try {
    ...
    // When superadmin changes a user's role, automatically verify them
    const updateData = { role, isVerified: true };
...
// Verify user (manually verify email for users)
router.put('/users/:id/verify', superAdminAuth, async (req, res) => {
  try {
    const { id } = req.params;
...
router.post('/users/admin', superAdminAuth, async (req, res) => {
  try {
    ...
    // Create admin user - automatically verified since superadmin creates it
    const adminUser = new User({
      name,
      email: email.toLowerCase(),
      password,
      role: 'admin',
      isVerified: true // Superadmin-created users are automatically verified
    });
...
// Create regular user (for superadmin)
router.post('/users/user', superAdminAuth, async (req, res) => {
  try {
    const { name, email, password } = req.body;
    ...
    // Create regular user - automatically verified since superadmin creates it
